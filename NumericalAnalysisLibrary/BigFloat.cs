...
namespace NumericalAnalysisLibrary
{
    ...
    public class BigFloat : IComparable, IComparable<BigFloat>, IEquatable<BigFloat>
    {
        ...
        public BigFloat(float value) : this(value.ToString())
        {
        }
        public BigFloat(double value) : this(value.ToString())
        {
        }
        ...
        {
            if (!trailingZeros && this == 0)
                return "0";
            else if (this == 0)
                return "0." + string.Join("", new int[precision]);
            ...
            while (precision-- > 0 && BigInteger.Abs(decimals) > 0)
            {
                sb.Append(BigInteger.Abs(decimals) % 10);
                decimals /= 10;
            }

            if (trailingZeros)
                return string.Format("{0}{1}.{2}", (result == 0 && remainder < 0 ? "-" : ""), result, new string(sb.ToString().Reverse().ToArray()));
            else
                return string.Format("{0}{1}.{2}", (result == 0 && remainder < 0 ? "-" : ""), result, new string(sb.ToString().Reverse().ToArray()).TrimEnd(new char[] { '0' }));
        ...
        public static BigFloat ParseExp(string value)
        {
            if (value == null)
                throw new ArgumentNullException("value");
            ...
            value = value.Replace(" ", "");
            value = value.Replace(',', '.');
            int pos = value.ToLower().IndexOf('e');

            BigFloat left = Parse(value.Substring(0, pos));

            int sign = value[pos + 1] == '-' ? -1 : 1;

            BigInteger power = BigInteger.Pow(10, Convert.ToInt32(value.Substring(pos + 2)));
